    ...
    <div class="max-w-7xl mx-auto mt-8 p-8 bg-white shadow-md rounded-lg">
        ...
        <form id="form-projeto" action="{{ route('projetos.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <fieldset class="mb-8">
                ...
                <input type="text" name="periodo" class="w-full border-gray-300 rounded-md mb-4" placeholder="Fevereiro a Junho de 2025." required {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}>

                <label class="block mb-2">Professor(es) envolvidos:</label>
                <div id="professores-wrapper">
                    <div class="mb-4">
                        <input type="text" name="professores[0][nome]" class="w-full border-gray-300 rounded-md mb-2" placeholder="Nome do professor" required {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}>
                        <input type="email" name="professores[0][email]" class="w-full border-gray-300 rounded-md mb-2" placeholder="Email (opcional)" {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}>
                        <input type="text" name="professores[0][area]" class="w-full border-gray-300 rounded-md" placeholder="Área (opcional)" {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}>
                    </div>
                </div>
                ...
                <div id="alunos-wrapper">
                    <div class="mb-4">
                        <input type="text" name="alunos[0][nome]" class="w-full border-gray-300 rounded-md mb-2" placeholder="Nome do aluno" required {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}>
                        <input type="text" name="alunos[0][ra]" class="w-full border-gray-300 rounded-md mb-2" placeholder="RA" required {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}>
                        <input type="text" name="alunos[0][curso]" class="w-full border-gray-300 rounded-md" placeholder="Curso" required {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}>
                    </div>
                </div>
                @if(!$disableAlunoFields)
                ...
                <textarea name="publico_alvo" class="w-full border-gray-300 rounded-md mb-4" placeholder="População em Geral" {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}></textarea>

                <label class="block mb-2">Data de Início:</label>
                <input type="date" name="data_inicio" id="data_inicio" class="w-full border-gray-300 rounded-md mb-4" required {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}>

                <label class="block mb-2">Data de Término:</label>
                <input type="date" name="data_fim" id="data_fim" class="w-full border-gray-300 rounded-md mb-4" required {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}>
            </fieldset>
            ...
            <fieldset class="mb-8">
                ...
                <textarea name="introducao" class="w-full border-gray-300 rounded-md mb-4" placeholder="Descreva os problemas observados em uma organização ou sociedade, a vantagem de fazer o projeto tanto para a equipe (alunos) e sociedade." {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}></textarea>

                <label class="block mb-2">2. Objetivos do Projeto</label>
                <textarea name="objetivo_geral" class="w-full border-gray-300 rounded-md mb-4" placeholder="Geral: o que pretende atingir." {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}></textarea>

                <label class="block mb-2">3. Justificativa</label>
                <textarea name="justificativa" class="w-full border-gray-300 rounded-md mb-4" placeholder="Justifique o porquê de desenvolver o projeto." {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}></textarea>

                <label class="block mb-2">4. Metodologia</label>
                <textarea name="metodologia" class="w-full border-gray-300 rounded-md mb-4" placeholder="Descrever brevemente como o projeto será conduzido." {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}></textarea>

                <label class="block mb-2">5. Atividades a serem desenvolvidas</label>
                <small class="block mb-2 text-gray-600">(Relacione todas as atividades que serão desenvolvidas - o que fazer, como fazer e carga horária).</small>

                <div id="atividades-wrapper">
                    <div class="mb-4">
                        <textarea name="atividades[0][o_que_fazer]" class="w-full border-gray-300 rounded-md mb-2" placeholder="O que fazer?" required {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}></textarea>
                        <textarea name="atividades[0][como_fazer]" class="w-full border-gray-300 rounded-md mb-2" placeholder="Como fazer?" required {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}></textarea>
                        <input type="number" name="atividades[0][carga_horaria]" class="w-full border-gray-300 rounded-md" placeholder="Carga horária" required {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}>
                    </div>
                </div>
                @if(!$disableAlunoFields)
                ...
                <div id="cronograma-wrapper">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <input type="text" name="cronograma[0][atividade]" class="form-control" placeholder="Título da Atividade" required {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}>
                        <select name="cronograma[0][mes]" class="form-control" required {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}>
                ...
                <textarea name="recursos" class="w-full border-gray-300 rounded-md mb-4" {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}></textarea>

                <label class="block mb-2">8. Resultados Esperados</label>
                <textarea name="resultados_esperados" class="w-full border-gray-300 rounded-md mb-4" {{ $disableAlunoFields ? 'readonly disabled class=opacity-50' : '' }}></textarea>
            ...
            <input type="text" name="numero_projeto" class="w-full border-gray-300 rounded-md mb-4" {{ $disableNapexFields ? 'readonly disabled class=opacity-50' : '' }}>

            <label class="block mb-2">Data recebimento da Proposta de Projeto de Extensão pelo NAPEx</label>
            <input type="date" name="data_recebimento_napex" class="w-full border-gray-300 rounded-md mb-4" {{ $disableNapexFields ? 'readonly disabled class=opacity-50' : '' }}>

            <label class="block mb-2">Data de Encaminhamento da Proposta do Projeto de Extensão para os devidos pareceres</label>
            <input type="date" name="data_encaminhamento_parecer" class="w-full border-gray-300 rounded-md mb-4" {{ $disableNapexFields ? 'readonly disabled class=opacity-50' : '' }}>

            <label class="block mb-2">Aprovação do NAPEx:</label>
            <div class="mb-4">
                <label><input type="radio" name="aprovado_napex" value="sim" {{ $disableNapexFields ? 'disabled' : '' }}> Sim</label>
                <label class="ml-4"><input type="radio" name="aprovado_napex" value="nao" {{ $disableNapexFields ? 'disabled' : '' }}> Não</label>
            </div>

            <label class="block mb-2">Exposição de motivos (NAPEx)</label>
            <textarea name="motivo_napex" class="w-full border-gray-300 rounded-md mb-4" {{ $disableNapexFields ? 'readonly disabled class=opacity-50' : '' }}></textarea>
            ...
            <div class="mb-4">
                <label><input type="radio" name="aprovado_coordenador" value="sim" {{ $disableCoordenadorFields ? 'disabled' : '' }}> Sim</label>
                <label class="ml-4"><input type="radio" name="aprovado_coordenador" value="nao" {{ $disableCoordenadorFields ? 'disabled' : '' }}> Não</label>
            </div>

            <label class="block mb-2">Exposição de motivos (Coordenador)</label>
            <textarea name="motivo_coordenador" class="w-full border-gray-300 rounded-md mb-4" {{ $disableCoordenadorFields ? 'readonly disabled class=opacity-50' : '' }}></textarea>

            <label class="block mb-2">Data do parecer do Coordenador</label>
            <input type="date" name="data_parecer_coordenador" class="w-full border-gray-300 rounded-md mb-4" {{ $disableCoordenadorFields ? 'readonly disabled class=opacity-50' : '' }}>
